...
### Lecture 4: "The Coefficients That Stayed Zero"

A large fraction of breast cancer patients who receive chemotherapy would have survived without it. In 2002, Laura van 't Veer used microarrays — roughly 25,000 genes measured on 78 patients — and a supervised filter of univariate gene-trait correlations to find a 70-gene signature predicting metastasis. The signature was commercialized as MammaPrint, cleared by the FDA in 2007, and validated in the 6,693-patient MINDACT trial: about 46% of high-risk patients were spared unnecessary chemotherapy. At Duke University, the oncologist Anil Potti built signatures from cell-line drug-response data that supposedly predicted which chemotherapy drug a tumor would respond to. Clinical trials enrolled patients based on these signatures. Two statisticians at MD Anderson, Keith Baggerly and Kevin Coombes, spent years trying to reproduce the analyses and couldn't — mislabeled samples, off-by-one errors in gene indices, conclusions opposite to what the data supported. More than a dozen papers were retracted or corrected; Potti's medical license was eventually surrendered. MammaPrint and the Duke metagenes were *not* the same statistical method (MammaPrint was not LASSO; the Duke work was its own variant), but they were the same *kind* of high-dimensional clinical prediction problem. The difference between them is the validation pipeline — external cohorts, open code and data, training/test separation — not the choice of regularizer. Today we learn the tools — ridge, LASSO, elastic net — and the discipline required to deploy them responsibly.
